...
#     start = time.time()
#
#     filepath_test = 'train_IQA2750/Imgs/' # Dataset to use.
#     data_store = {}
#     data_store['input'] = []
#     data_store['gt'] = []
#     data_store['dense'] = []
...
#     train_size = 8 #72 #The number of training images.
#     test_size = 2 #8 #The number of test images.
...
#     samples_per_epoch = 16 #For stochastic gradient descent, the number of batches per epoch.
#     image_pool_size = train_size + test_size
#     input_seq = np.arange(image_pool_size)
...
#     file_model = "saved_models/IQA2750_" + str(i)
#     filepath_label = 'train_IQA2750/Masks/'
#
#     for i in range(image_pool_size):
#         n = str(i).zfill(4)
#
#         #Open intensity image.
#         img = Image.open(filepath_test + n + '.jpg').getdata()
...
#         img = Image.open(filepath_label + n + '.png').getdata()
#         # print(np.array(img).shape)
...
#     checkpoint = ModelCheckpoint(file_model + ".hdf5",  monitor='loss', verbose=1, save_best_only=save_best_only, mode='min')
#
#
...
#     pickle.dump(hist.history, open(file_model + '.his', "wb"))
#
#     end = time.time()
#     print(f'Time for training {i+1}  is {end - start}')
#
#
# """## Making predictions"""
#
...
# experiment = 'IQA2750_'
#
# imported_pickle = pickle.load(open(model_path + '/' + experiment + '1.his', "rb"))
#
# parameters = imported_pickle['parameters']
# file_path = 'train_IQA2750/'
# data_store = {}
# data_store['input'] = []
# data_store['gt'] = []
# data_store['dense'] = []
...
#
#     # Open intensity image.
#     img = Image.open(file_path + 'Test/' + n + '.jpg').getdata()
#     wid, hei = img.size
#     temp = np.array(img).reshape((hei, wid, 3))[:, :, 2].astype(np.float32)
#
#     data_store['input'].append(temp)
#
#     # Open ground-truth image.
#     img = Image.open(file_path + 'Label/' + n + '.jpg').getdata()
#     data_store['gt'].append(np.array(img).reshape((hei, wid))[:, :].astype(np.float64))
#
#     # Filter ground-truth image to produce density kernel representation
#     data_store['dense'].append(ndimage.filters.gaussian_filter(data_store['gt'][i], sigma, mode='constant'))
...
#
#         inpu_sum = np.sum(inpu_img) / 255.0 / 61.
#         pred_sum = np.sum(pred_img) / 255.0 / 61.
#
...
# Visualization
for img_name in [i for i in os.listdir("out_imgs") if (i.split('.')[-1] == 'p' and i[:4] == 'inpu')]:
    ...
    plt.suptitle(img_name + "\nActual: " + str(len(ori_contours)) + "\nPrediction: " + str(len(pre_contours)))
    plt.subplot(1, 2, 1)
    plt.title("Original Label")
    plt.imshow(ori_image, cmap='gray')
    plt.subplot(1, 2, 2)
    plt.title("Prediction Estimate Density Map")
...
#     # if len(gray.shape) == 3:
#     #     # Split the image into its color channels and select the first channel
#     #     _, gray, _ = cv2.split(gray)
#
...
#     plt.suptitle(img_name + "\nActual: " + str(len(ori_contours)) + "\nPrediction: " + str(len(pre_contours)))
#     plt.subplot(1, 2, 1)
#     plt.title("Original Density Image")
#     plt.imshow(ori_image, cmap='gray')
#     plt.subplot(1, 2, 2)
